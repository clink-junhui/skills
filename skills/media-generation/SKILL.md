...
6. Wait for the payment layer's structured `payment_handoff`. In generic `clink-payment-skill`, parse `clink-cli pay` by exit code and `data.status`; for `exit=0` + `status=1`, build the handoff from the pay result. For explicit Alipay `status=5` plus `customerAction.type=QR_CODE_REQUIRED`, remember that Codex command transcripts may be collapsed and are not user-visible. When stderr contains QR block characters and no terminal warning, extract only the contiguous QR lines and repeat them exactly in the next user-visible assistant message inside a fenced `text` block. Do not replace successful character output with PNG because it might look misaligned, and do not call `nodeRepl.emitImage`, `view_image`, or another image/file attachment tool in this branch. Use `customerAction.imagePath` only when the exact terminal warning appears or no block characters were produced. Do not retry pay or call `check_recharge_status` yet. Wait for one correlated `agent_order.succeeded,agent_order.failed` any-of event. Other async/3DS paths also wait for the matching order result before continuing.
